...
extern size_t eco_createGate( char* name ,unsigned type, unsigned old );
extern void eco_addFanin( size_t pEcoGate, char* fanin );
extern void	eco_addPO( char* name, unsigned old );
size_t pEcoGate;	// to store eco gate
extern int eco_flag_parse;
extern void eco_read();
...
//            RetValue = Ver_ParseGate( pMan, pNtkTemp );
	...
	#ifdef ECO_LIB
	if ( eco_flag_parse == 1 ) eco_flag_parse = 2;
	// allen added;
	#endif
    ...
    while ( 1 )
    {
        ...
        if ( nMsb == -1 && nLsb == -1 )
        {
            ...
            if ( SigType == VER_SIG_INPUT || SigType == VER_SIG_INOUT )
			{
				...
				#ifdef ECO_LIB
				if ( eco_flag_parse ) pEcoGate = eco_createGate( pWord, (unsigned)NTK_GATE_PI, eco_flag_parse==1 );
				#endif
			}
            if ( SigType == VER_SIG_OUTPUT || SigType == VER_SIG_INOUT )
			{
				...
				#ifdef ECO_LIB
				if ( eco_flag_parse ) eco_addPO( pWord, eco_flag_parse==1 );
				#endif
			}
...
//                sprintf( Buffer, "%s[%d]", pWord, Bit );
                ...
                if ( SigType == VER_SIG_INPUT || SigType == VER_SIG_INOUT ) 
				{
					...
					// allen added
					#ifdef ECO_LIB
					if ( eco_flag_parse )pEcoGate = eco_createGate( Buffer, (unsigned)NTK_GATE_PI, eco_flag_parse==1 );
					#endif
				}
                if ( SigType == VER_SIG_OUTPUT || SigType == VER_SIG_INOUT )
				{
					...
					// allen added
					#ifdef ECO_LIB
					if ( eco_flag_parse ) eco_addPO( Buffer, eco_flag_parse==1 );
					#endif
...
//    }
    ...
    while ( 1 )
    {
		...
		if( eco_flag_parse )
		{
			pEcoGate = eco_createGate( pWord, (unsigned)NTK_GATE_BUF, eco_flag_parse==1 );
			Vec_PtrPush( vEcoGates, pEcoGate );
		}
		#endif
        ...
        if ( nMsb >= 0 && nLsb >= 0 && Limit > 1 )
        {
            ...
            for ( i = 0, Bit = nLsb; i < Limit; i++, Bit = nMsb > nLsb ? Bit + 1: Bit - 1  )
            {
				...
				if( eco_flag_parse )
				{
					printf( "adding constant for assign\n");
					pEcoGate = Vec_PtrEntry( vEcoGates, i );
					eco_addFanin( pEcoGate, Abc_ObjName(pNet) );
				}
				#endif
...
if( eco_flag_parse)
{
	printf( "parseAssign: unknown if statement 1\n" );
}
#endif
...
#ifdef ECO_LIB
// add fanin
if( eco_flag_parse && Vec_PtrSize(pMan->vNames)==0)
{
	// printf( "parseAssign: vNames is empty (%s, %s, %d)\n", pWord, Abc_ObjName(pNet), Vec_PtrSize( vEcoGates ) );
	pEcoGate = Vec_PtrEntry( vEcoGates, 0 );
	eco_addFanin( pEcoGate, pWord );
}
#endif
...
#ifdef ECO_LIB
// add fanin
if(Vec_PtrSize(pMan->vNames)==0)
{
	//printf( "parseAssign: empty vNames. Given name: pNode=%s, pNet=%s\n", Abc_ObjName(pNode), Abc_ObjName(pNet));
	Abc_ObjAssignName( pNode, Abc_ObjName(pNet), "" );
	//printf( "parseAssign: empty vNames. New name: pNode=%s, pNet=%s\n", Abc_ObjName(pNode), Abc_ObjName(pNet));
}
else if( eco_flag_parse )
{
	//printf( "parseAssign: not empty vNames. Given name: pNode=%s, pNet=%s\n", Abc_ObjName(pNode), Abc_ObjName(pNet));
}
#endif
...
//                pNet = Ver_ParseFindNet( pNtk, pName );
				...
				if( eco_flag_parse )
				{
					//printf( "parseAssign: adding %s as a fanin of %s\n", Abc_ObjName(pNet), Abc_ObjName(Abc_ObjFanout0(pNode)) );
					pEcoGate = Vec_PtrEntry( vEcoGates, i );
					eco_addFanin( pEcoGate, pName );
				}
				#endif
            }
        }
