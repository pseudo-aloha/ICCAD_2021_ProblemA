...
#define ECO_LIB
#ifdef ECO_LIB
extern eco_mergeGate( char* name, size_t dirver, unsigned complement, int flag );
extern int eco_flag_fraig;
#endif
...
// allen added
	...
	if ( eco_flag_fraig )
	{
		...
		/*
		pNode = Abc_AigConst1( pNtkNew );
		name = Abc_ObjName( pNode );
		printf("calling eco function: %s\t%x\t%d\n", name, pNode, 0 );
		eco_mergeGate( "", (size_t)pNode, 1, -1 );

		Abc_NtkForEachPi( pNtkNew, pNode, i )
		{
			name = Abc_ObjName( pNode );
			printf("calling eco function: %s\t%x\t%d\n", name, pNode, 0 );
			eco_mergeGate( "", (size_t)pNode, 0, i );
		}
		*/

		Abc_NtkForEachPo( pNtkNew, pNode, i )
		{
			name = Abc_ObjName( pNode );
			// printf("calling eco function: %s\t%x\t%d\n", name, Abc_ObjFanin0(pNode), Abc_ObjFaninC0(pNode) );
			eco_mergeGate( name, (size_t)Abc_ObjFanin0(pNode), Abc_ObjFaninC0(pNode), -2 );
		}
	}
#endif
...
// allen test
//printf( "Create ntk node after fraig: %x\t%x\n", pNodeFraigR, pRes);
//pRes->myData = ((Fraig_NodeStruct_t*)pNodeFraigR)->pData1;
    return Abc_ObjNotCond( pRes, Fraig_IsComplement(pNodeFraig) );
}
