...
// allen test =========================================
// check all new nodes here
// TODO: store net name to a vector
// TODO: record if the net is complemented
	...
	if ( eco_flag_strash ) 
	{
		...
		{
			name = Abc_ObjName(pNode);		// netname

			// if #fanin == 0, it's a reduced assign
			...
			//printf( "strash: adding net name %s (%x) (#fanin: %d)\n", name, pNtk, Abc_ObjFaninNum(pNode));

			pCopy = (Abc_Obj_t*)((size_t)(pNode->pCopy) & ~01);
			complement = (Abc_Obj_t*)((size_t)(pNode->pCopy) & 01);

			// create vector of vector of char
			if ( !pCopy ) 
			{
				// printf("%s has no corresponding new node!!\n", Abc_ObjName(pNode) );
				continue;
			}

			if ( pCopy -> myData == 0 ) pCopy -> myData = Vec_PtrAlloc(8);

			pStr = Vec_StrAlloc(8);

			if ( complement ) Vec_StrPush( pStr, '~' );
			Vec_StrAppend( pStr, name );
			Vec_StrPush( pStr, '\0' );
			Vec_PtrPush( pCopy -> myData, pStr );


			//printf( "create node after strash: %s\t%x\t(myData: %s)\n", name, pNode->pCopy, Vec_StrArray( Vec_PtrEntryLast(pCopy->myData) ) );
		}
	}

	// printf( "abcstrash done\n");
...
//ABC_PRT( "Time", Abc_Clock() - clk );
    Vec_PtrForEachEntry( Abc_Obj_t *, vNodes, pNodeOld, i )
    {
        ...
        // allen test: strash node copy here
