...
if ( eco_flag_strash )
{
	...
	if ( pNode->myData != 0 )
	{
		...
		int j;
		Vec_PtrForEachEntry( void*, pVec, pStr, j)
		{
			str = Vec_StrArray( pStr );
			// printf( "rev, const: creating PO in miter for %s\n", str );

			pNet = Abc_NtkCreatePo( pNtkMiter );
			Abc_ObjAssignName( pNet, (str[0]=='~') ? str+1: str, SUFFIX_OLD );
			Abc_ObjAddFanin( pNet, (str[0] == '~') ? Aig_Not(pCopy) : pCopy );
		}
	}
	Abc_NtkForEachCi( pNtk1, pNode, i )
	{
		// myData == null:
		// means that the net is not in our circuit
		// and it's some internal signal in aig
		if ( pNode->myData == 0 ) continue;		

		pVec = pNode->myData;
		pCopy = pNode->pCopy;

		if ( Abc_AigNodeIsConst(pCopy) ) continue;

		int j;
		Vec_PtrForEachEntry( void*, pVec, pStr, j)
		{
			str = Vec_StrArray( pStr );
			// printf( "rev, ci: creating PO in miter for %s\n", str );
	...
	Abc_AigForEachAnd( pNtk1, pNode, i )
	{
		...
		Vec_PtrForEachEntry( void*, pVec, pStr, j)
		{
			...
			pNet = Abc_NtkCreatePo( pNtkMiter );
			// printf( "rev, and: creating PO in miter for %s\n", str );
	...
	if ( pNode->myData != 0 )
	{
		...
		{
			str = Vec_StrArray( pStr );
			// printf( "gol, const: creating PO in miter for %s\n", str );

			pNet = Abc_NtkCreatePo( pNtkMiter );
			Abc_ObjAssignName( pNet, (str[0]=='~') ? str+1: str, SUFFIX_GOL );
			Abc_ObjAddFanin( pNet, (str[0] == '~') ? Aig_Not(pCopy) : pCopy );
		}
	}
	...
	Abc_NtkForEachCi( pNtk2, pNode, i )
	{
		...
		{
			str = Vec_StrArray( pStr );
			//printf( "gold, ci: creating PO in miter for %s\n", str );
	...
	Abc_AigForEachAnd( pNtk2, pNode, i )
	{
		...
		{
			str = Vec_StrArray( pStr );
			// printf( "gol, and: creating PO in miter for %s\n", str );
...
void Abc_NtkMiterAddOne( Abc_Ntk_t * pNtk, Abc_Ntk_t * pNtkMiter )
{
    ...
    Abc_AigForEachAnd( pNtk, pNode, i )
	{
		...
		if ( pNode -> pCopy -> pNtk != pNtkMiter ) printf( "%s WTF\n", Abc_ObjName(pNode));
